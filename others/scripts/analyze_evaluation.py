#!/usr/bin/env python
"""
SCI-ARC Evaluation Analysis Script.

Generates detailed analysis including:
1. Per-task visualizations (input, target, prediction)
2. Transformation type analysis (where SCI-ARC succeeds/fails)
3. Error pattern analysis
4. HTML report for easy browsing

Usage:
    python scripts/analyze_evaluation.py --results results/sci_arc_default_eval
    python scripts/analyze_evaluation.py --results results/sci_arc_default_eval --generate-html
"""
...
# Known ARC transformation categories (heuristic classification)
TRANSFORMATION_CATEGORIES = {
    'size_change': 'Output size differs from input size',
    'color_change': 'Colors change but pattern preserved',
    'rotation': 'Grid appears rotated',
    'reflection': 'Grid appears reflected/flipped',
    'translation': 'Pattern shifted in grid',
    'scaling': 'Pattern scaled up or down',
    'fill': 'Regions filled with color',
    'pattern_completion': 'Partial pattern completed',
    'object_manipulation': 'Objects moved, copied, or modified',
    'counting': 'Output depends on counting elements',
    'symmetry': 'Symmetry-based transformation',
    'unknown': 'Transformation type unclear',
}
...
def classify_transformation(input_grid: List, target_grid: List) -> str:
    ...
    This is a simple classifier - can be improved with more sophisticated analysis.
    """
...
<head>
    <meta charset="UTF-8">
    <title>SCI-ARC Evaluation Report</title>
    <style>
...
<body>
    <h1>SCI-ARC Evaluation Report</h1>
...
'''
    ...
    if analysis['background_collapse'] > analysis['total_tasks'] * 0.5:
        html += '''
    <div class="warning">
        <strong>Warning:</strong> High rate of background collapse detected ({:.1f}% of predictions are mostly background).
...
'''.format(100 * analysis['background_collapse'] / analysis['total_tasks'])
    
    # Transformation type analysis
    html += '''
    <div class="summary">
        <h2>Transformation Type Analysis</h2>
        <p>SCI-ARC's structural encoder should excel at recognizing transformation patterns. Here's how it performs by transformation type:</p>
...
'''
    ...
    for i, pred in enumerate(sorted_preds[:max_visualizations]):
        ...
        <div class="task-card {status_class}" data-correct="{filter_class}">
            ...
            <p>Pixel Accuracy: {pixel_acc*100:.1f}% | Transformation: {transform_type.replace('_', ' ').title()}</p>
            <div class="grids">
...
def main():
    parser = argparse.ArgumentParser(description='Analyze SCI-ARC evaluation results')
    ...
    print("=" * 60)
    print("SCI-ARC Evaluation Analysis")
    print("=" * 60)
    ...
    print(f"Correct: {analysis['correct_tasks']} ({100*analysis['correct_tasks']/analysis['total_tasks']:.1f}%)" if analysis['total_tasks'] > 0 else "Correct: 0")
    print(f"Incorrect: {analysis['incorrect_tasks']}")
    print(f"Background Collapse Cases: {analysis['background_collapse']}")
    
    print("\n--- Transformation Type Analysis ---")
    print("(SCI-ARC's structural encoder should excel at recognizing these patterns)")
    print()
