# =============================================================================
# TRM Original Implementation - trm.py (Tiny Recursive Reasoning Model)
...
class TinyRecursiveReasoningModel_ACTV1InnerCarry:
    """Inner carry state for the TRM model."""
...
class TinyRecursiveReasoningModel_ACTV1Carry:
    """Complete carry state for the TRM ACT wrapper."""
...
class TinyRecursiveReasoningModel_ACTV1Config(BaseModel):
    ...
    H_layers: int  # ignored in main TRM
    L_layers: int
    ...
    puzzle_emb_len: int = 16  # if non-zero, specified to this value
    no_ACT_continue: bool = True  # No continue ACT loss, only use sigmoid of halt
...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    """Inner model for TRM with H and L levels."""
    ...
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        # Reasoning Layers - shared L_level for both H and L updates
    ...
    def _input_embeddings(self, input: torch.Tensor, puzzle_identifiers: torch.Tensor):
        """Generate input embeddings with optional puzzle embeddings and position encodings."""
        # Token embedding
    ...
    def empty_carry(self, batch_size: int):
        """Create empty carry state for a batch."""
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        """Forward pass with H and L cycle processing."""
        seq_info = dict(
        ...
        # H_cycles-1 without grad (memory efficient training)
        with torch.no_grad():
        ...
        # 1 with grad (only last H step has gradients)
        for _L_step in range(self.config.L_cycles):
            z_L = self.L_level(z_L, z_H + input_embeddings, **seq_info)
        z_H = self.L_level(z_H, z_L, **seq_info)
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    """ACT wrapper for TRM with Q-learning based halting."""
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        Implements Q-learning based halting decision for adaptive computation.
        """
        ...
        with torch.no_grad():
            ...
            if self.training and (self.config.halt_max_steps > 1):
                ...
                # Halt signal
                # NOTE: During evaluation, always use max steps, this is to guarantee 
                # the same halting steps inside a batch for batching purposes
